...
function Vehiculos() {
  ...
  const obtenerClientes = async () => {
    const respuesta = await axios.get('http://192.168.100.66:3001/clientes')
  ...
  const agregarVehiculo = async (e) => {
    e.preventDefault()
    await axios.post('http://192.168.100.66:3001/vehiculos', formulario)
    setFormulario({ cliente_id: '', patente: '', marca: '', modelo: '', anio: '', color: '' })
    obtenerVehiculos()
  }

  const eliminarVehiculo = async (id) => {
    await axios.delete(`http://192.168.100.66:3001/vehiculos/${id}`)
    obtenerVehiculos()
  }
