...
function CobrosSeguro() {
  ...
  const obtenerTodo = async () => {
    const [c, p, a, s] = await Promise.all([
      axios.get('http://192.168.100.66:3001/cobros-seguros'),
      axios.get('http://192.168.100.66:3001/cobros-seguros/proximos'),
      axios.get('http://192.168.100.66:3001/aseguradoras'),
      axios.get('http://192.168.100.66:3001/siniestros')
    ])
    setCobros(c.data)
    setProximos(p.data)
  ...
  const obtenerCalendario = async () => {
    const respuesta = await axios.get(`http://192.168.100.66:3001/cobros-seguros/calendario?mes=${mes}&anio=${anio}`)
  ...
  const agregarCobro = async (e) => {
    e.preventDefault()
    await axios.post('http://192.168.100.66:3001/cobros-seguros', formulario)
  ...
  const marcarCobrado = async (id) => {
    await axios.patch(`http://192.168.100.66:3001/cobros-seguros/${id}/cobrar`)
    obtenerTodo()
  }
