...
function Articulos() {
  ...
  const obtenerArticulos = async () => {
    const respuesta = await axios.get('http://192.168.100.66:3001/articulos')
  ...
  const agregarArticulo = async (e) => {
    e.preventDefault()
    await axios.post('http://192.168.100.66:3001/articulos', formulario)
    setFormulario({ nombre: '', descripcion: '', precio: '', stock: '' })
    obtenerArticulos()
  }

  const eliminarArticulo = async (id) => {
    await axios.delete(`http://192.168.100.66:3001/articulos/${id}`)
    obtenerArticulos()
  }
